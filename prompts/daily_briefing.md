...
- **confidence** 置信度（1-10）：你对这条分析的把握有多高
  ...
  - 5-6：有一定推测成分
  - 1-4：信息不完整，需后续跟踪
- **actionability** 行动等级：ignore / watch / prepare / act（每条只能选一个，不要全部选 watch）
  - `act`：建议 1-2 个交易日内处理（止损/止盈/紧急调整）。例子：持仓个股暴雷、达到止损点、政策突变改变投资逻辑
  - `prepare`：未来几天可能需行动（如：今晚美联储决议→明天关注A股反应）。例子：重要数据发布前夕、即将到来的解禁潮
  - `watch`：加入观察列表，月度体检时再评估。例子：行业趋势变化、间接影响的事件
  - `ignore`：纯背景信息或与家庭无关，无需后续关注
  - ⚠️ 约束：ignore 应占 50% 以上，act 仅在极端情况下使用。每批简报中 watch+prepare+act 合计不超过 8 条
- **impacted_assets** 受影响的资产：仅输出具体的持仓代码（如 510300、588000、518880）或明确资产类别（银行股、房贷、黄金ETF），禁止输出"家庭日常开支""消费"等非投资类表述。无相关持仓时输出空数组 []
...
1. `top_brief_ids` 选择最值得家庭关注的 3 条，不要包含 `actionability == "ignore"` 的条目。同一事件组（相同 `duplicate_group`）只选分数最高的一条。
2. 如果多条新闻是同一事件，`duplicate_group` 必须一致，前端会合并展示。
3. `impacted_assets` 只写代码或资产类别（如 `510300`、`银行股`、`房贷`），禁止写"家庭日常开支""消费"等模糊表述。无具体资产时写 `[]`。
